...
fn test_vk_same() {
    ...
    let prover1 = MockProver::<_>::run(*DEGREE as u32, &dummy_circuit, dummy_circuit.instance()).unwrap();
    let prover2 = MockProver::<_>::run(*DEGREE as u32, &real_circuit, real_circuit.instance()).unwrap();
    ...
    for i in 0..fixed1.len() {
        for j in 0..fixed1[i].len() {
            if fixed1[i][j] != fixed2[i][j]  {
                log::error!("fixed assignment not same, {}th fixed column, {}th row. {:?} vs {:?}", i, j,  fixed1[i][j], fixed2[i][j]);
            }
        }
    }
